...
export const Route = createFileRoute("/sell")({
  head: () => ({ meta: [{ title: "Sell on Purana Bazaar" }] }),
  component: Sell,
});
...
function Sell() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) { toast.info("Please sign in to post"); navigate({ to: "/login" }); return; }
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-md mx-auto py-32 text-center px-4">
          <h1 className="font-serif text-4xl text-primary">Sign in to sell</h1>
  ...
  if (!authLoading && !!user && !profileLoading && !isAdmin) {
    return (
      <div className="min-h-screen">
        <Header />
        <div className="max-w-2xl mx-auto py-28 text-center px-4">
          <span className="text-xs uppercase tracking-[0.3em] text-gold">Sell now</span>
          <h1 className="font-serif text-4xl md:text-5xl text-primary mt-2">Coming soon</h1>
          <p className="text-muted-foreground mt-4">Seller posting is temporarily available for admins only while we prepare the next public release.</p>
          <div className="mt-6 flex gap-2 justify-center">
            <Button variant="hero" asChild><Link to="/browse">Browse listings</Link></Button>
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-4 md:px-8 py-12">
        <div className="text-center max-w-2xl mx-auto">
          <span className="text-xs uppercase tracking-[0.3em] text-gold">Post your ad</span>
          <h1 className="font-serif text-4xl md:text-5xl text-primary mt-2">List your item in 60 seconds</h1>
          <p className="text-muted-foreground mt-3">Free for individuals. Reach thousands of buyers across Pakistan today.</p>
        </div>
        ...
        <form onSubmit={submit} className="mt-12 space-y-6 rounded-3xl bg-card border border-border p-6 md:p-10 shadow-soft">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-border">
            ...
            <Button variant="hero" size="lg" type="submit" disabled={submitting}><Upload /> {submitting ? "Publishing…" : "Publish listing"}</Button>
          </div>
        </form>
      </div>
      <Footer />
